...
afterFFT=MyFFT(origin);
%自己编写方法处理
afterDream_dp=dream_dp(afterFFT); 

origin1=rgb2gray(imread('666.jpg'));  %图片2
afterFFT1=MyFFT(origin1);
afterDream_gp=dream_gp(afterFFT1); 

pngDP=real(MyIFFT(ifftshift(afterDream_dp))); %获取图片1低频-反傅里叶变换展示图片(real获取实部数据)
pngGP=real(MyIFFT(ifftshift(afterDream_gp))); %获取图片2高频-反傅里叶变换展示图片(real获取实部数据)
% 官方方式处理
