...
img_sobelAllTemp=img_sobelAll;
subplot(2,3,3),imshow(img_sobelAllTemp),title("sobel未施加阈值");
%测试使用
...
%imgST=atan2(img_sobelX,img_sobelY);
imgSTTemp=atan2(img_sobelY,img_sobelX);
%开始过滤非最大值(非最大抑制) 
%双阈值限制 
TLow=0.1*max(max(imgSTTemp));%低阈值
THre=0.3*max(max(imgSTTemp));%高阈值
for r=2:rows-1
    %for r=2:rows-1
    for c=2:cols-1
        %for c=2:cols-1
        ...
        a9=temp(9);
        %判断梯度方向
        %水平边缘 22.5=pi/8    0.39269908169872 0.7>0.39<1.17
        %  112.5=5*pi/8        1.9634954084936
        %  157.5=7*pi/8        2.7488935718911
        %  67.5=3*pi/8         1.1780972450962
...
subplot(2,3,4),imshow(img_sobelAll),title("Canny算子后的图像");
img_sobelGF=edge(origin,'canny');
subplot(2,3,5),imshow(img_sobelGF),title("Canny官方算子后的图像");
